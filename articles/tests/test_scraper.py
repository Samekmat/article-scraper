...
class DateParsingIntegrationTest(SimpleTestCase):
    def parse(self, raw, base=None, lang=["pl", "en"]):
        ...
        parsed = dateparser.parse(raw, languages=lang, settings={"RELATIVE_BASE": base})
        if parsed:
    ...
    def test_should_parse_english_relative_hours(self):
        ...
        expected = (base - timedelta(hours=3)).replace(
...
class ScrapeArticleSeleniumTest(TestCase):
    ...
    def test_should_extract_domain_from_url(self, mock_get_driver):
        ...
        mock_get_driver.return_value = mock_driver
        mock_driver.page_source = (
            "<html><head><title>Test</title></head><body>{}</body></html>".format(
                "B" * 250
            )
        )
        article = scrape_article_selenium(
            "https://subdomain.example.com/path/article?param=value"
        )
